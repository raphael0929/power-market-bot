...
사용법:
  python daily_agent.py generate   # AI 초안 생성 → drafts/ 저장
  python daily_agent.py publish    # approved/ 파일 → Blogger + YouTube 게시
"""
import json
import sys
...
def cmd_generate() -> None:
    """AI로 오늘의 초안을 생성하고 drafts/ 폴더에 저장합니다."""
    ...
    print()
    print("👉 내용을 검토한 뒤 approved/ 폴더로 파일을 이동하면 자동 게시됩니다.")


def cmd_publish() -> None:
    """approved/ 폴더의 오늘 날짜 파일을 Blogger와 YouTube에 게시합니다."""
    ...
    approved_file = APPROVED_DIR / f"{today}.json"

    if not approved_file.exists():
        print(f"⚠️  승인된 파일이 없습니다: {approved_file}")
        print("   drafts/ 의 파일을 approved/ 로 이동한 뒤 재실행하세요.")
        sys.exit(0)

    content = json.loads(approved_file.read_text(encoding="utf-8"))
    blog = content["blog"]
    yt = content["youtube"]
